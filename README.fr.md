...
La plateforme est déployée sur trois machines virtuelles :

```
┌────────────────────────────────────────────────────────────────────┐
│                     VM 1 : Serveur Airflow                         │
...
│  • Scripts ETL Python                                              │
│  • Conteneurs Docker                                               │
│  • Se connecte à : JSearch API, PostgreSQL (VM2), Superset (VM3)   │
└────────────────┬───────────────────────────────────────────────────┘
                 │
                 │ Extraction, Transformation, Chargement
                 ▼
┌────────────────────────────────────────────────────────────────────┐
│                   VM 2 : Serveur PostgreSQL                        │
│                      (Entrepôt de Données)                         │
├────────────────────────────────────────────────────────────────────┤
│  • Base de données PostgreSQL 16                                   │
│  • Schéma en étoile (fact_job_post + dimensions)                   │
│  • Stockage des offres d'emploi                                    │
│  • Sert les données à : Airflow (VM1), Superset (VM3)              │
└────────────────────────────────────────────────────────────────────┘
                 │
                 │ Requêtes & Visualisation
                 ▼
┌────────────────────────────────────────────────────────────────────┐
│                 VM 3 : Serveur Apache Superset                     │
│                  (Couche BI & Visualisation)                       │
├────────────────────────────────────────────────────────────────────┤
│  • Tableaux de bord Apache Superset                                │
│  • Graphiques & Rapports interactifs                               │
│  • SQL Lab pour analyses ad-hoc                                    │
│  • Lit les données depuis : PostgreSQL (VM2)                       │
└────────────────────────────────────────────────────────────────────┘
```

**Détails de l'Infrastructure :**
- **VM 1 (Airflow)** : Orchestre les workflows ETL, extrait les données de JSearch API, transforme et charge vers PostgreSQL
- **VM 2 (PostgreSQL)** : Entrepôt de données centralisé avec modèle dimensionnel (schéma en étoile)
- **VM 3 (Superset)** : Plateforme de business intelligence pour l'exploration et la visualisation des données
